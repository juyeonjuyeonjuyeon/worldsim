"""
WS_forest_rain v015
사용자 피드백: "구름과 나무의 움직임이 없다" -> 추가.

바람(WIND_SPEED, _WIND_DX/_WIND_DY)이 이미 빗방울 궤적에는 적용돼 있었는데
나무/구름은 가만히 있던 모순을 해결:
1. 나무 흔들림: 트렁크는 고정하고 가지+잎 전체를 트렁크 위쪽 한 피벗
   Empty에 부모 연결, 그 피벗을 사인파로 살짝 회전시켜 흔들림 표현
   (실제 나무의 외팔보 굽힘을 본/소프트바디 없이 표현할 수 있는 가장 단순한
   근사 — 정확한 연속 굽힘은 아님을 인지하고 있음). 트리마다 정수 진동
   사이클(3~7회)과 진폭(작은 나무일수록 더 크게 흔들림, 진폭 ∝ 1/scale)을
   다르게 줘서 숲 전체가 똑같이 흔들리지 않게 함.
2. 구름 흐름: 구름 볼륨 재질에 TexCoord(Object)+Mapping 노드를 추가해
   노이즈 텍스처 좌표를 매 프레임 갱신 -> 구름이 흐르는 것처럼 보임.
   둘 다 영상이 CYCLE_SEC 주기로 무한 반복되게 설계돼 있어서, 한쪽으로
   계속 흘리면(또는 계속 같은 방향으로 회전시키면) 루프 경계에서 위치가
   안 맞아 이어붙임 자국이 생김 -> 정수 사이클의 사인파 왕복으로 표현해
   루프 시작/끝이 정확히 같은 상태가 되도록 함.

v014 기반 유지 (아래):
사용자 요청: "더 실제 같은 요소 추가 및 수정할 것 찾아봐" -> 6개 발견,
5개 적용(캐노피 차단은 복잡도 대비 효과가 낮아 보류).

적용한 5개:
1. 바람: lateral_drift() 추가 — 낙하와 동일한 항력 시간상수(tau=v_t/g)를
   재사용해 빗방울이 옆으로 밀리는 정도를 계산(WIND_SPEED=1.6m/s, 30도
   방향). 빗줄기 꼬리 방향도 수평+수직 합성 속도로 다시 계산.
2. 나무 습윤도: 비가 진행될수록 나무 재질(trunk/branch/leaf) roughness가
   서서히 낮아져(0.9→0.25) 비에 젖은 광택 표현.
3. 그림자 부드럽게: sun.energy 1.0→0.4, sun.angle 5°→20°(면적이 넓은
   광원처럼 동작해 구름에 산란된 부드러운 빛 표현), 보조광(sl) 60→90으로
   보완.
4. 웅덩이 반사 복원: roughness 0.9→0.18로 낮춰 하늘/구름이 웅덩이 표면에
   비치게 함(이전엔 과다 반사 방지로 높여놨던 것).
5. 지면 안개: 낮은 높이(z=0~2.5m, 카메라 z=6.5보다 낮게)의 옅은 볼류메트릭
   박스 추가, 원경에 공기 원근감(aerial perspective) 표현. 처음엔 8m
   두께+카메라가 안개 속에 들어가 화면 전체가 두꺼운 안개로 뒤덮이는
   문제가 있었음 -> 높이를 낮춰 카메라 아래로 빼서 해결.
...
# ─── 바람 (횡방향 항력 평형) ───
# 지금까지는 "바람 없음"을 가정해 빗줄기가 완벽히 수직이었음 — 내부적으로는
# 일관됐지만, 실제 야외에서 비가 한 점 바람도 없이 내리는 경우는 거의 없어
# 부자연스럽게 "멈춘" 느낌이 났음. 트릭이 아니라 같은 항력 모델을 횡방향에
# 적용: 항력은 상대속도에 반비례해 작용하므로, 수평속도는 시간상수
# tau=v_t/g(수직 종단속도 공식에서 이미 쓴 값)로 바람속도에 지수적으로
# 근접함 — vx(t)=W(1-exp(-t/tau)). 큰(무거운) 방울은 tau가 커서 바람의
# 영향을 더 천천히 받고(현실과 동일), 작은 방울은 빠르게 바람과 같이 움직임.
WIND_SPEED = 1.6   # m/s, 약한 바람(미풍) — 산들바람 수준
_WIND_DX = math.cos(math.radians(30))   # 바람 방향 단위벡터 (수평면 내 30도)
_WIND_DY = math.sin(math.radians(30))

# 나무 흔들림/구름 흐름도 같은 바람(_WIND_DX, _WIND_DY 방향)에 반응하게 함 —
# 빗방울만 옆으로 날리고 나무/구름은 가만히 있으면 모순. 단, 둘 다 영상이
# CYCLE_SEC 주기로 무한 반복되도록 만들어졌으므로(빗방울과 동일한 설계),
# 일정한 방향으로 계속 흘러가면 루프 경계에서 위치가 안 맞아 이어붙임
# 자국이 생김 -> 정수 사이클의 사인파 진동(왕복)으로 표현해 매끈하게 반복.
CLOUD_DRIFT_AMP    = 4.0   # 구름 노이즈 좌표 흔들림 폭 (Object 좌표계, m)
CLOUD_DRIFT_CYCLES = 1     # 루프(CYCLE_SEC) 안에서 왕복 횟수

def lateral_drift(t, vt, wind=WIND_SPEED):
    """t초 동안 바람에 의해 옆으로 밀린 거리 (같은 항력 시간상수 재사용)"""
    tau = vt / G
    return wind * (t - tau * (1.0 - np.exp(-t / tau)))
...
# =============================================
puddle_mat = make_mat("Puddle", (0.03, 0.06, 0.08, 1), roughness=0.18,
                      transmission=0.0, ior=1.333)
# 완전히 평평한 표면은 "판떼기" 같아 보임 -> 약한 잔물결 범프로 표면에
# 미세한 변화를 줘서 매끈한 플라스틱이 아니라 물처럼 보이게 함.
# 이전엔 roughness=0.9(거의 무광)라 웅덩이가 하늘을 안 비춰 칙칙해 보임 —
# 그때는 하늘이 밋밋해서 반사를 올리면 하얗게 떴지만, 지금은 구름 질감이
# 생겨서 반사율을 0.18로 낮춰도(매끈하게) 하늘/구름이 자연스럽게 비침.
...
# =============================================
# 나무 22그루
# 비가 계속 내리면 나무껍질/잎도 젖어서 광택이 생기는데, 지금까지는
# 지면만 시간에 따라 마름->젖음이 바뀌고 나무는 끝까지 마른 매트 그대로
# 였음. 같은 시간 램프를 나무 재질의 roughness에도 적용 (트래킹용 목록).
# =============================================
_tree_mats = []   # (material, 원래 마른 roughness) - 프레임 핸들러가 매 프레임 갱신
_tree_sway = []   # (피벗 Empty, 루프 내 진동 사이클 수, 진폭, 위상) - 가지/잎 흔들림용
...
def make_tree(x, y, scale, seed):
    ...
    # ─── 바람에 흔들리는 가지/잎 피벗 ───
    # 실제 나무는 트렁크 하단은 거의 안 움직이고 위쪽일수록 더 많이
    # 휘는 외팔보(cantilever) 굽힘인데, 본/소프트바디 변형 없이 정확히
    # 재현하긴 어려움 -> 가지+잎 전체를 트렁크 위쪽 한 피벗점 기준으로
    # 함께 회전시키는 단순화된 근사(트렁크 자체는 고정해 뿌리 박힌 느낌 유지).
    ...
    # 트리별로 다른 진동 사이클 수(루프 안에서 정수번 왕복 -> 매끈한 반복)와
    # 진폭(작고 가는 나무일수록 더 많이 흔들림)을 부여해 모든 나무가 똑같이
    # 흔들리지 않게 함(실제로도 나무 크기/형태마다 고유 진동수를 가짐).
...
def update_scene(scene):
    ...
    drift  = lateral_drift(t_eff, v_terminal)      # 바람에 의한 횡방향 이동(실제 항력 평형)
    ...
    # 빗줄기 꼬리: 이제 수직 속도 + 바람에 의한 수평 속도를 합친 실제
    # 속도 방향으로 기움 (가짜 wind_tilt 상수가 아니라 같은 항력 모델의
    # 결과). 길이는 속도 크기에 비례.
    vx, vy, vz = wind_now * _WIND_DX, wind_now * _WIND_DY, speed
    vmag = np.sqrt(vx ** 2 + vy ** 2 + vz ** 2)
    vmag_safe = np.where(vmag < 1e-6, 1.0, vmag)
    vis_len = np.clip(_slen * (vmag / 4.0), _slen * 0.3, _slen * 2.0)
    ...
    # 기반 크기 — 실제 운동량에 비례하는 진짜 물리값. 위치도 바람으로
    # 드리프트된 실제 착지 지점(x,y) 사용.
    ...
    wetness_ramp = min(1.0, elapsed / _GROUND_RAMP_FRAMES)
    ground_wetness_value.outputs[0].default_value = wetness_ramp

    # ── 나무도 같은 램프로 비에 젖어 광택이 생김 (roughness 점차 감소) ──
    for _mat, _dry_r in _tree_mats:
        _mat.node_tree.nodes["Principled BSDF"].inputs["Roughness"].default_value = (
            _dry_r - wetness_ramp * (_dry_r - 0.25))

    # ── 구름 흐름: 같은 바람 방향으로 노이즈 좌표를 사인파로 왕복시켜 구름이
    # 흐르는 것처럼 보이게 함 (한쪽으로만 계속 흘리면 7초 루프 경계에서
    # 텍스처가 어긋나 이어붙임 자국이 생기므로 정수 사이클 왕복으로 매끈하게 반복).
    cloud_drift = CLOUD_DRIFT_AMP * math.sin(2 * math.pi * CLOUD_DRIFT_CYCLES * t / CYCLE_SEC)
    cloud_mapping.inputs["Location"].default_value = (
        cloud_drift * _WIND_DX, cloud_drift * _WIND_DY, 0.0)

    # ── 나무 흔들림: 바람이 빗방울만 밀고 나무는 가만히 있으면 모순이므로,
    # 가지/잎 피벗을 같은 바람 방향으로 살짝 기울임. 트리별 정수 사이클로
    # 진동시켜 루프 경계에서도 매끈하게 반복.
    for _pivot, _n_cyc, _amp, _phase in _tree_sway:
        _angle = _amp * math.sin(2 * math.pi * _n_cyc * t / CYCLE_SEC + _phase)
        _pivot.rotation_euler[0] = -_angle * _WIND_DY
        _pivot.rotation_euler[1] = _angle * _WIND_DX
...
print("프레임 핸들러 등록 완료")
# 핸들러 즉시 실행은 구름/안개(cloud_mapping 등) 생성 이후로 이동 — 거기서 호출
...
# =============================================
# 조명
# 구름층을 추가했는데 태양이 여전히 또렷한 그림자를 만들어 비현실적이었음.
# 실제로 구름이 덮이면 직사광이 산란되어 그림자가 부드럽고 옅어짐 ->
# 태양 각크기(angle)를 키워 반음영을 넓히고 에너지를 낮춤, 대신 하늘에서
# 오는 확산광(AREA)을 키워 전체 밝기는 비슷하게 유지.
# =============================================
bpy.ops.object.light_add(type='SUN', location=(0, 0, 10))
sun = bpy.context.active_object
sun.data.energy = 0.4                 # 1.0 -> 0.4: 직사광 비중 축소
sun.data.angle  = math.radians(20)     # 5 -> 20: 구름 산란으로 그림자 퍼짐
sun.rotation_euler[0] = math.radians(70)
sun.rotation_euler[2] = math.radians(100)

bpy.ops.object.light_add(type='AREA', location=(0, 0, 15))
sl = bpy.context.active_object
sl.data.energy = 90                    # 60 -> 90: 확산광 비중 확대 (전체 밝기 보완)
sl.data.size   = 20
...
_ctexcoord = _cnodes.new("ShaderNodeTexCoord")
cloud_mapping = _cnodes.new("ShaderNodeMapping")   # 프레임 핸들러가 Location을 갱신 -> 구름이 흐르는 것처럼 보임
_clinks.new(_ctexcoord.outputs["Object"], cloud_mapping.inputs["Vector"])

_ctex = _cnodes.new("ShaderNodeTexNoise")
_ctex.inputs["Scale"].default_value = 2.5
_ctex.inputs["Detail"].default_value = 4.0
_clinks.new(cloud_mapping.outputs["Vector"], _ctex.inputs["Vector"])
...
# =============================================
# 지면 실안개 (대기 흡광/aerial perspective)
# 비 오는 날 특유의 옅은 수증기 안개가 없어서 먼 곳이 부자연스럽게 깨끗하게
# 보임. 구름과 같은 볼륨 산란 방식이지만 지면 가까이, 훨씬 낮은 밀도로 —
# 먼 나무/지형이 가까운 것보다 살짝 흐려지고 하늘색에 묻히는 효과.
# =============================================
bpy.ops.mesh.primitive_cube_add(size=1, location=(0, 0, 1.25))
mist_obj = bpy.context.active_object
mist_obj.name = "GroundMist"
mist_obj.scale = (200, 200, 2.5)   # 카메라(z~6.5)보다 낮게 -> 안 그래야
                                    # 카메라가 안개 속에 들어가 시야 전체가
                                    # 두꺼운 안개로 뒤덮이는 문제 방지
bpy.ops.object.transform_apply(scale=True)

mist_mat = bpy.data.materials.new("GroundMistVolume")
mist_mat.use_nodes = True
_mnodes = mist_mat.node_tree.nodes
_mlinks = mist_mat.node_tree.links
for n in list(_mnodes): _mnodes.remove(n)

_mvol = _mnodes.new("ShaderNodeVolumePrincipled")
_mvol.inputs["Color"].default_value = (0.82, 0.85, 0.86, 1.0)
_mvol.inputs["Density"].default_value = 0.012   # 박스가 얕아졌으니(2.5m) 밀도는 비슷하게 유지
_mout = _mnodes.new("ShaderNodeOutputMaterial")
_mlinks.new(_mvol.outputs["Volume"], _mout.inputs["Volume"])
...
# 핸들러 즉시 실행 (현재 프레임 반영) — cloud_mapping 등 모든 노드가 생성된 뒤에 호출
update_scene(scene)

# =============================================
# 프리뷰 렌더 (프레임 60) – 애니메이션 전 품질 확인
# =============================================
preview_path = r"C:\Users\kkjjy\Documents\WorldSim\output\WS_forest_rain_v015_preview.png"
...
# =============================================
frame_dir = r"C:\Users\kkjjy\Documents\WorldSim\output\WS_forest_rain_v015"
os.makedirs(frame_dir, exist_ok=True)

print(f"\n{n_frames}프레임 애니메이션 렌더링 시작...")
for f in range(1, n_frames + 1):
    scene.frame_set(f)   # frame_change_post 핸들러 실행
    out = os.path.join(frame_dir, f"WS_forest_rain_v015_{f:04d}.png")
...
bpy.ops.wm.save_as_mainfile(
    filepath=r"C:\Users\kkjjy\Documents\WorldSim\WS_forest_rain_v015.blend"
)
print("씬 저장: WS_forest_rain_v015.blend")
