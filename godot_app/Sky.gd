...
var _eclipse_manual: bool = false # true면 슬라이더가 진행도 직접 제어(자동 증가 정지)
var eclipse_status: String = ""  # Main.gd가 읽어 상태표시줄에 표시
# 블루문: 산불·화산재의 ~1µm 에어로졸이 빨강을 산란 제거 → 달이 청색으로 보이는 희귀현상
var _blue_moon: bool = false
...
# ── UI용 명시적 setter (체크박스 on/off + 슬라이더 강도) ───────────────
# 토글이 아니라 명시 상태 → UI 체크박스와 동기화(누를 때마다 뒤집힘 방지).
func set_aurora(on: bool) -> void:
	_aurora_force = on
	if on:
		if _aurora_kp <= 0.0: _aurora_kp = 5.0   # 켤 때 기본 KP
		_aurora_next_event = 99999.0

func set_aurora_kp(kp: float) -> void:
	_aurora_kp = clampf(kp, 0.0, 9.0)

func set_eclipse(type: String, on: bool) -> void:
	if on:
		_eclipse_test = type
		# 슬라이더로 진행도를 직접 제어하지 않는 한, 켜는 즉시 개기(1.0)로 시작
		if not _eclipse_manual: _eclipse_test_t = 1.0
	else:
		if _eclipse_test == type:
			_eclipse_test = ""

func set_eclipse_progress(t: float) -> void:
	_eclipse_test_t = clampf(t, 0.0, 1.0)
	_eclipse_manual = true

func set_blue_moon(on: bool) -> void:
	_blue_moon = on

func set_comet(on: bool) -> void:
	_comet_test_mode = on
	if not on:
		_comet_nuc_inst.visible  = false
		_comet_ion_inst.visible  = false
...
func _update_sky_and_lights(sun_altaz: Vector2, moon: Dictionary, cloud_props: Dictionary, lightning_flash: float, delta: float) -> void:
	...
	if _eclipse_test != "":
		# 수동(슬라이더) 제어가 아니면 0→1 자동 진행(데모 애니메이션)
		if not _eclipse_manual:
			_eclipse_test_t = minf(_eclipse_test_t + delta * 0.15, 1.0)
	else:
		_eclipse_test_t = 0.0
		_eclipse_manual = false   # 꺼지면 수동 제어 해제
	...
	elif _blue_moon and moon_alt > -2.0:
		eclipse_status = "블루문 (에어로졸 산란)"
	...
	# 블루문: ~1µm 에어로졸이 빨강을 산란 제거 → 적색 채널 억제, 청색 상대 강조.
	if _blue_moon:
		moon_lit_c = Vector3(moon_lit_c.x * 0.45, moon_lit_c.y * 0.85, moon_lit_c.z * 1.15)
